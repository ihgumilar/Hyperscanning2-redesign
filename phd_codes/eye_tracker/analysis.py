...
class EyeAnalysis:
    ...
    def combine_dataframe(self, path2files: str, tag: str):
        ...
        """
        Objective  : Combine dataframe for either odd or even subject.
                    It also involves pre-processing
                    (replacing missing values with average value of columns
                    where they are).

        Parameters : - path2files (str): Path to a directory where all cleaned eye tracker files are stored
                     - tag (str): eye gaze condition, ie. averted_pre, averted_post, direct_pre, direct_post, natural_pre, natural_post

        Output:      - df_odd (dataframe)
                    - df_even(dataframe)
