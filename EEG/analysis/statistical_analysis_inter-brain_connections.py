...
for i in tqdm(range(begin, end, step), desc="Please, listen 2 music & have some coffee..."):
    ...
    psd1 = analyses.pow(load_epoch_S1, fmin=4, fmax=40,
                        n_fft=1000, n_per_seg=1000, epochs_average=True)
    psd2 = analyses.pow(load_epoch_S2, fmin=4, fmax=40,
                        n_fft=1000, n_per_seg=1000, epochs_average=True)
    ...
    theta_ccorr, alpha_ccorr, beta_ccorr, gamma_ccorr = ground_result_ccorr[:, 0:n_ch, n_ch:2 * n_ch]
    theta_plv, alpha_plv, beta_plv, gamma_plv = ground_result_plv[:, 0:n_ch, n_ch:2 * n_ch]
    theta_coh, alpha_coh, beta_coh, gamma_coh = ground_result_coh[:, 0:n_ch, n_ch:2 * n_ch]

    # ground truth matrix using ccorr
    ccorr_combined_ground_truth_matrices = [theta_ccorr, alpha_ccorr, beta_ccorr, gamma_ccorr]
    ...
    ccorr_combined_freq_n_connections = [ccorr_theta_n_connections,ccorr_alpha_n_connections,ccorr_beta_n_connections,ccorr_gamma_n_connections]
    ...
    plv_combined_freq_n_connections = [plv_theta_n_connections,plv_alpha_n_connections,plv_beta_n_connections,plv_gamma_n_connections]
    ...
    coh_combined_freq_n_connections = [coh_theta_n_connections,coh_alpha_n_connections,coh_beta_n_connections,coh_gamma_n_connections]
    ...
    for participant1_channel in range(len(ch_names)):
        for participant2_channel in range(len(ch_names)):
            ...
            k_ccorr_theta_permuted = [] # initialising list that will store all the ccor values (ccorr value range of 0 to +1). its length should equal n_perms
            k_ccorr_alpha_permuted = [] 
            k_ccorr_beta_permuted = [] 
            k_ccorr_gamma_permuted = [] 
            
            k_plv_theta_permuted = [] # initialising list that will store all the plv values (plv value range of 0 to +1). its length should equal n_perms
            k_plv_alpha_permuted = [] 
            k_plv_beta_permuted = [] 
            k_plv_gamma_permuted = [] 
            
            k_coh_theta_permuted = [] # initialising list that will store all the coh values (coh value range of 0 to +1). its length should equal n_perms
            ...
            for iperm in range(n_perms):
                ...
                # for participant 1
                perm1 = rng.permutation(len(epoch1)) # randomising indices without replacement
                epoch1_in_permuted_order = [epoch1[i] for i in perm1] # index the epoch to get permuted epochs
                ...
                complex_signal = analyses.compute_freq_bands(data_inter_permuted, sampling_rate,
                                                 freq_bands)
                ...
                ccorr_theta_permuted, ccorr_alpha_permuted, ccorr_beta_permuted, ccorr_gamma_permuted = result_ccorr[:, 0:n_ch, n_ch:2 * n_ch]
                plv_theta_permuted, plv_alpha_permuted, plv_beta_permuted, plv_gamma_permuted = result_plv[:, 0:n_ch, n_ch:2 * n_ch]
                coh_theta_permuted, coh_alpha_permuted, coh_beta_permuted, coh_gamma_permuted = result_coh[:, 0:n_ch, n_ch:2 * n_ch]
                ...
                combined_k_ccorr_frequency_permuted = [k_ccorr_theta_permuted, k_ccorr_alpha_permuted, k_ccorr_beta_permuted, k_ccorr_gamma_permuted]
                ...
                combined_k_plv_frequency_permuted = [k_plv_theta_permuted, k_plv_alpha_permuted, k_plv_beta_permuted, k_plv_gamma_permuted]
                ...
                combined_k_coh_frequency_permuted = [k_coh_theta_permuted, k_coh_alpha_permuted, k_coh_beta_permuted, k_coh_gamma_permuted]
            
            # iterate each theta, alpha, beta, gamma 
            for iterate_each_freq in range(len(combined_k_ccorr_frequency_permuted)): # 4 because we have classified frequency range into 4 types: theta, alpha, beta, gamma
                
                # Calculate p value 
                z_value = 1.96 # equivalent to p value of 0.05
                
                # calculate mean and standard deviation for each frequency band using ccorr
                ccorr_mean_permuted  = np.mean(combined_k_ccorr_frequency_permuted[iterate_each_freq])
                ccorr_std_permuted = np.std(combined_k_ccorr_frequency_permuted[iterate_each_freq])
                ccorr_z_score = (ccorr_combined_ground_truth_matrices[iterate_each_freq][participant1_channel][participant2_channel]-ccorr_mean_permuted)/ccorr_std_permuted
                ...
                if np.abs(ccorr_z_score) > z_value:
                    ...
                    ccorr_combined_freq_n_connections[iterate_each_freq][participant1_channel][participant2_channel] = 1
                ...
                plv_mean_permuted  = np.mean(combined_k_plv_frequency_permuted[iterate_each_freq])
                plv_std_permuted = np.std(combined_k_plv_frequency_permuted[iterate_each_freq])
                plv_z_score = (plv_combined_ground_truth_matrices[iterate_each_freq][participant1_channel][participant2_channel]-plv_mean_permuted)/plv_std_permuted
                # checking if the z score is greater than z value (p < .05) i.e. if the connection is significant or not. this serves as a marker of significant connection between two electrodes
                if np.abs(plv_z_score) > z_value:
                    # assign value 1 if the connection between ch_names[participant1_channel] and  ch_names[participant2_channel] is significant, otherwise 0 by default
                    # this is a 4 x 16 x 16 array. 1st index identifies which frequency band it is (e.g. theta, alpha, beta, gamma), and 2nd index represent the electrode of first participant, and 3rd index represent the electrode of second participant 
                    plv_combined_freq_n_connections[iterate_each_freq][participant1_channel][participant2_channel] = 1
                    # print(f"{combined_ground_truth_matrices_names[iterate_each_freq]} connection between Participant1 {ch_names[participant1_channel]} and Participant2 {ch_names[participant2_channel]} is significant")
                
                # calculate mean and standard deviation for each frequency band using coh
                coh_mean_permuted  = np.mean(combined_k_coh_frequency_permuted[iterate_each_freq])
                coh_std_permuted = np.std(combined_k_coh_frequency_permuted[iterate_each_freq])
                coh_z_score = (coh_combined_ground_truth_matrices[iterate_each_freq][participant1_channel][participant2_channel]-coh_mean_permuted)/coh_std_permuted
                # checking if the z score is greater than z value (p < .05) i.e. if the connection is significant or not. this serves as a marker of significant connection between two electrodes
                if np.abs(coh_z_score) > z_value:
                    # assign value 1 if the connection between ch_names[participant1_channel] and  ch_names[participant2_channel] is significant, otherwise 0 by default
                    # this is a 4 x 16 x 16 array. 1st index identifies which frequency band it is (e.g. theta, alpha, beta, gamma), and 2nd index represent the electrode of first participant, and 3rd index represent the electrode of second participant 
                    coh_combined_freq_n_connections[iterate_each_freq][participant1_channel][participant2_channel] = 1
                    # print(f"{combined_ground_truth_matrices_names[iterate_each_freq]} connection between Participant1 {ch_names[participant1_channel]} and Participant2 {ch_names[participant2_channel]} is significant")
    ...
    saved_filename1 = saved_directory + "Pre_ccorr_combined_pair_S" + str(i+1) + "_and_S" + str(i+2) + "_connection_data.pkl"
    with open(saved_filename1, 'wb') as handle:
        pickle.dump(ccorr_combined_freq_n_connections_list, handle,protocol=pickle.HIGHEST_PROTOCOL)
        
    # save plv connection data for a pair 
    saved_filename2 = saved_directory + "Pre_plv_combined_pair_S" + str(i+1) + "_and_S" + str(i+2) + "_connection_data.pkl"
    with open(saved_filename2, 'wb') as handle:
        pickle.dump(plv_combined_freq_n_connections_list, handle,protocol=pickle.HIGHEST_PROTOCOL)
    
    # save coh connection data for a pair 
    saved_filename3 = saved_directory + "Pre_coh_combined_pair_S" + str(i+1) + "_and_S" + str(i+2) + "_connection_data.pkl"
    with open(saved_filename3, 'wb') as handle:
        pickle.dump(coh_combined_freq_n_connections_list, handle,protocol=pickle.HIGHEST_PROTOCOL)
